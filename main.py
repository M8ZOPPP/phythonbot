...
BOT_TOKEN = "5351715496:AAHXNk2mGHclZnoFFgNcNctFjK2ZQHtvb00"
...
CHANNELS = ["@dailyearningkaro"] #add channels to be checked here in the format - ["Channel 1", "Channel 2"] 
...
def menu(id):
    ...
    keyboard.row('ðŸ™ŒðŸ» Referrals', 'ðŸŽ Bonus', 'ðŸ’¸ Withdraw')
    keyboard.row('âš™ï¸ Set Wallet', 'ðŸ“ŠStatistics')
    bot.send_message(id, "*ðŸ¡ Home*", parse_mode="Markdown",
   ...
   try:
    ...
    if msg == '/start':
        ...
        markup.add(telebot.types.InlineKeyboardButton(
           text='ðŸ¤¼â€â™‚ï¸ Joined', callback_data='check'))
        ...
        if ch == True:
            ...
            if user not in data['refer']:
                ...
                if int(data['referby'][user]) != user_id:
                    ...
                    data['referred'][ref] += 1
                    bot.send_message(
                        ref_id, f"*ðŸ§ New Referral on Level 1, You Got : +{Per_Refer} {TOKEN}*", parse_mode="Markdown")
                    json.dump(data, open('users.json', 'w'))
...
def send_text(message):
   try:
    ...
    if message.text == 'ðŸ™ŒðŸ» Referrals':
        data = json.load(open('users.json', 'r'))
        ref_msg = "*â¯ï¸ Total Invites : {} Users\n\nðŸ‘¥ Refferrals System\n\n1 Level:\nðŸ¥‡ LevelÂ°1 - {} {}\n\nðŸ”— Referral Link â¬‡ï¸\n{}*"
    ...
    if message.text == "âš™ï¸ Set Wallet":
        ...
        keyboard = telebot.types.ReplyKeyboardMarkup(True)
        keyboard.row('ðŸš« Cancel')
        send = bot.send_message(message.chat.id, "_âš ï¸Send your paytm Wallet Address._",
                                parse_mode="Markdown", reply_markup=keyboard)
        # Next message will call the name_handler function
        bot.register_next_step_handler(message, trx_address)
    if message.text == "ðŸŽ Bonus":
        user_id = message.chat.id
        user = str(user_id)
        cur_time = int((time.time()))
        data = json.load(open('users.json', 'r'))
        #bot.send_message(user_id, "*ðŸŽ Bonus Button is Under Maintainance*", parse_mode="Markdown")
    ...
    if message.text == "ðŸ“ŠStatistics":
        user_id = message.chat.id
        user = str(user_id)
        data = json.load(open('users.json', 'r'))
...
def trx_address(message):
   try:
    ...
    if len(message.text) == 34:
        ...
        bot.send_message(message.chat.id, "*ðŸ’¹Your Trx wallet set to " +
...
def amo_with(message):
   try:
    ...
    if msg.isdigit() == False:
        bot.send_message(
            user_id, "_ðŸ“› Invaild value. Enter only numeric value. Try again_", parse_mode="Markdown")
        return
